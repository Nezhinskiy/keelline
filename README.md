...
| `.keelline/local/artifacts.json` | The record of the bytes Keelline last wrote under `.keelline/local/artifacts/`, so an unedited copy is refreshed or retired and an edited one is left. Git-ignored and never committed; deleting it only sends later runs back to comparing with what they render | the scaffold engine; `keelline uninstall` removes it |

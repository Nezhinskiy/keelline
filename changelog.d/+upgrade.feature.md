...
them does. A project recording a newer Keelline is refused, and nothing is written when the
plan refuses anything. A file that cannot be written part-way through stops the run with
what was done kept and recorded, and the next run continues from there.

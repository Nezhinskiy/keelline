...
**Spec:** the agent-harness extraction design, at the source's spec path — a private repository; it cannot be opened from this one — §7.1 (the
...
```
Expected: no hits. Scoped to this lane's own files on purpose: a neighbour's hit is a

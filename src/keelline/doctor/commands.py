...
all — the Codex hook-trust hash the cross-harness contract lists as unmeasured — so an exit code
that counted skips would make `doctor` red on every correct installation. Two others were
counted here and are not: `files` until the release lane shipped the record it compares
against, and `ci-ref` until `init` shipped the writer of `[ci] ref`. `ci-ref`'s skip is now a
*state* — this repository records no ref — and which state is ordinary moves with the release
history and not with any code here, so nothing in this module asserts a number about it: before
a released tag matches the Keelline running there is no commit for `init` to record, and after
one there is. Exit 1, the findings exit code, is reserved for `red`, and `warn` does not reach
it either: a budget lowered below the preset and a harness link the trust gate has not opened
are both correct states somebody should still see.
...
**The remedies live in `--json` and never in the summary.** Every command gets one output line,

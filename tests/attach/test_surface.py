...
def test_the_surface_carries_what_every_downstream_lane_reaches_for() -> None:
    ...
    # lets an export arrive unnoticed. `doctor` reads the ledger and the binding; `setup` runs
    # `check` and reads its `Result`. No mutation entry: the mutation is adding an export.
    required = {
        "BOUND",
        "LEDGER",
        "LOCAL_SETTINGS",
...
def test_the_export_list_is_exactly_what_the_module_imports_from_this_lane() -> None:
    ...
    tree = ast.parse(Path(attach.__file__).read_text(encoding="utf-8"))
    for node in ast.walk(tree):
        assert not isinstance(node, ast.Import), "the surface re-exports, it does not import"
        if not isinstance(node, ast.ImportFrom) or node.module == "__future__":
            continue
        assert node.module is not None and node.module.startswith("keelline.attach.")
        imported |= {alias.asname or alias.name for alias in node.names}
    assert imported == set(attach.__all__)
    for name in attach.__all__:
        assert hasattr(attach, name)


def test_every_type_the_surface_names_in_a_signature_is_on_the_surface() -> None:
    # Derived rather than restated, exactly as `tests/memory/test_surface.py` derives it: the
    # two tests above cannot catch a name nobody added to either side, because one is a list a
    # person maintains and the other compares the module against itself. `Attached` grew a
    # `links` field in the middle of this wave, which is the shape this catches.
    import dataclasses
    import inspect
    import typing
    ...
    def area_types(annotation: object) -> set[str]:
        found: set[str] = set()
        if isinstance(annotation, type) and annotation.__module__.startswith("keelline.attach"):
            found.add(annotation.__name__)
        for argument in typing.get_args(annotation):
            found |= area_types(argument)
        return found

    exported = set(attach.__all__)
    missing: dict[str, set[str]] = {}
    for name in sorted(exported):
        thing = getattr(attach, name)
...
def test_the_surface_is_a_module_not_the_package_init() -> None:
    ...
    import keelline.attach

    init = Path(next(iter(keelline.attach.__path__))) / "__init__.py"

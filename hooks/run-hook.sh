...
# script's own path, the interpreter list is honoured only from an interactive terminal, `git`
# is asked with an allowlisted environment, and `CLAUDE_PROJECT_DIR` stays a destination — but a
# destination that cannot be entered is a token rather than silence.
#
...
#
# `env -i` with PATH and HOME, which is `gitenv.GIT_ENV_KEEP` minus the three locale names this
# query has no use for. `hooks/dispatch._git_toplevel` scrubs the identical call one layer down
...
# GIT_WORK_TREE=repoB <wrapper>` put the launcher in repoB. This is the Codex hot path, where
# `CLAUDE_PROJECT_DIR` is unset and `git` is the only answer.
root="${CLAUDE_PROJECT_DIR:-}"
[ -n "$root" ] || root=$(env -i PATH="${PATH:-}" HOME="${HOME:-}" git rev-parse --show-toplevel 2>/dev/null || true)
...
project=
if [ -n "$root" ]; then
  cd "$root" 2>/dev/null || fail "KL_NO_ROOT the project root this entry was given cannot be entered"
  project=$(pwd -P)
fi
...
#
# **An empty `project` means the candidate stands.** There is nothing to compare against: no
...
in_project() {
  ...
  [ -n "$dir" ] || return 1
  [ "$dir" = "$project" ] && return 0
  [ "${dir#"$project"/}" != "$dir" ] && return 0
  return 1
}
...
for c in $candidates; do
  ...
  [ -n "$resolved" ] || continue
  if in_project "$resolved"; then continue; fi
...
# nothing was found.
[ -n "$p" ] || fail "KL_NO_PY no python3 of 3.11 or newer among the candidates outside the project root"
